...
class XTBScanApp(wx.App):
    ...
    def OnInit(self):
        ...
        self.current_input_xyz_coordinates: Optional[np.ndarray] = None
        self.current_result_csv_file: Optional[Path] = None
        self.current_result_xyz_file: Optional[Path] = None
        self.current_result_xyz_atoms: Optional[np.ndarray] = None
    ...
    def set_events(self):
        ...
        self.button_view_save_selected.Bind(wx.EVT_BUTTON, self.on_button_view_save_selected)
        ...
        self.button_constrain_add.Bind(wx.EVT_BUTTON, self.on_button_constrain_add)
        self.button_constrain_remove.Bind(wx.EVT_BUTTON, self.on_button_constrain_remove)
    ...
    def load_result_csv_file(self, file: Path):
        ...
        xyz_file = file.absolute().parent / (file.stem + '.xyz')
    ...
    def load_result_xyz_file(self, file: Path):
        self.current_result_xyz_file = file.absolute()
    ...
    def on_button_view_save_selected(self, event):
        if self.current_result_xyz_coordinates is None or self.current_result_xyz_atoms is None:
            self.logging('No valid xyz file.')
            return
        ...
        dialog = wx.FileDialog(None, 'save file name',
                               ...
                               style=wx.FD_SAVE)
        dialog.SetDirectory(str(self.current_result_xyz_file.parent))
        if dialog.ShowModal() == wx.ID_OK:
            file = dialog.GetPath()
            dialog.Destroy()
        else:
        ...
        xyzutils.save_xyz_file(xyz_file=Path(file), atoms=self.current_result_xyz_atoms,
                               coordinates=self.current_result_xyz_coordinates[num-1], title='generated by xtbscan')
    ...
    def on_button_scan_get_current(self, event):
        ...
        for i in atoms:
            if i >= len(self.current_input_xyz_coordinates):
                self.logging('Input coordinates only have {} atoms.'.format(len(self.current_input_xyz_coordinates)))
                return
    ...
    def on_button_constrain_add(self, event):
        ctype = self.choice_constrain_type.GetStringSelection()
    ...
    def on_button_constrain_remove(self, event):
    ...
    def on_button_run(self, event):
        if self.calculation_thread is not None:
            ...
            if job_name == '':
                job_name = self.current_input_xyz_file.stem + '_xtbscan'
    ...
    def on_calc_end(self, event: CalcEndEvent):
        ...
        if event.success:
            self.logging('Calculation successfully finished: {}'.format(job_name))
            ...
            self.logging('Calculation failed or terminated: {}'.format(job_name))


def scan_validation(stype: str, atoms: str, start: str, end: str, num_step: str, coordinates=None) -> List[str]:
    """
...
def constrain_validation(ctype: str, atoms: str, value: str, coordinates=None) -> List[str]:
    """
    if input are not valid, return error messages in list [str].
    If ok, return empty list [].
    """
